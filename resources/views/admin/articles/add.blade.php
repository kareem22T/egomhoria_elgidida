    ...
    <div  v-if="categories_data && categories_data.length > 0">
        <div>
            ...
            <div class="mb-5 w-100 d-flex gap-3 justify-content-center">
                <button type="submit" class="btn btn-primary w-25 form-control button input" style="height: fit-content" @click="getContent().then(() => { add(title, content, preview_img, cat_id, author_name, tags)})"><i class="ti ti-plus"></i> اضافة الخبر</button>
                <button type="submit" class="btn btn-primary w-25 form-control button-gray input" style="height: fit-content" @click="getContent().then(() => { add(main_name, title_translations, content_translations, preview_img, cat_id, tags)})"><i class="ti ti-plus"></i> حفظ كمسودة</button>
            </div>
        </div>
    </div>
</div>
...
createApp({
  ...
  methods: {
    ...
    async add(title, content, thumbnail, cat_id, author_name, tags) {
      $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.post(`/elgomhoria/admin/articles/add`, {
                title: title,
                content: content,
                thumbnail: thumbnail,
                cat_id: cat_id,
    ...
    async uploadImage(image) {
        $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.post(`/elgomhoria/admin/images/upload`, {
                img: image,
            },
            {
                headers: {
    ...
    photoChanges(event) {
        ...
        if ($.inArray(fileType, validImageTypes) < 0) {
            ...
            $("#preview").attr(
                "src",
                "/elgomhoria/dashboard/images/add_image.svg"
            );
