...
<div class="card w-100" id="word_prev">
    <div class="card-header d-flex justify-content-between gap-3">
        ...
        <a href="/elgomhoria/admin/articles/add" class="btn btn-primary w-fit d-flex gap-2 align-items-center">
            ...
            <tr v-for="(article, index) in articles_data" :key="index">
                ...
                <td class="border-bottom-0">
                    <div class="d-flex gap-2">
                        <a :href="`/elgomhoria/admin/articles/edit/${article.id}`" class="btn btn-secondary p-2"><h4 class="ti ti-edit text-light m-0 fw-semibold"></h4></a>
...
createApp({
    ...
    methods: {
        ...
        async deletearticle(article_id) {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/elgomhoria/admin/articles/delete`, {
        ...
        async getarticles() {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/elgomhoria/admin/articles?page=${this.page}`, {
                },
                );
                if (response.data.status === true) {
                    $('.loader').fadeOut()
        ...
        async getSearch(search_words) {
            try {
                const response = await axios.post(`/elgomhoria/admin/articles/search?page=${this.page}`, {
