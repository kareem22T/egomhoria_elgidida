            ...
            <div>
                ...
                <div class="pop-up show-images-pop-up card" v-if="showImages" style="min-width: 90vw;height: 90vh;padding: 20px;display: flex;flex-direction: column;justify-content: space-between;gap: 1rem;">
                    ...
                    <div class="imgs p-2 gap-3" v-if="images && images.length" style="display: flex;grid-template-columns: repeat(auto-fit, minmax(250px, 1fr));flex-wrap: wrap;height: 100%;overflow: auto;">
                        <div class="img" @click="this.choosed_img = '{{ asset('/storage/app/public/uploads/') }}'  + '/' + img.path" v-for="(img, index) in images" :key="img.id" style="width: 260px;height: 230px;overflow: hidden;padding: 10px;border-radius: 1rem;box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;">
                            <img :src="'{{ asset('/storage/app/public/uploads/') }}'  + '/' + img.path" id="preview" alt="img logo" style="width: 100%;height: 100%;object-fit: contain;">
                        </div>
                    </div>
                ...
                <div class="w-50 mb-5" style="margin-left: auto;margin-right: auto">
                    <div @click="this.showImages = true; this.current_article_id = null" class="w-100 h-100 p-3 d-flex justify-content-center align-items-center form-control input" style="height: 170px; max-height: 300px">
                        <img :src="preview_img ? preview_img  : '/elgomhoria/public/dashboard/images/add_image.svg'" id="preview" alt="img logo" style="width: 100%;object-fit: contain; height: 100%;">
                    </div>
                </div>

                <div class="mb-5 w-100 d-flex gap-3 justify-content-center">
                    <button type="submit" class="btn btn-primary w-25 form-control button input" style="height: fit-content" @click="getContent().then(() => { save(title, content, preview_img, cat_id, tags, this.author)})">حفظ الخبر</button>
                    <button type="submit" class="btn btn-primary w-25 form-control button-gray input" style="height: fit-content" @click="getContent().then(() => { add(main_name, title_translations, content_translations, preview_img, cat_id, tags)})">حفظ كمسودة</button>
                </div>
            </div>
        </div>
    @endif
...
createApp({
  ...
  methods: {
    async save(title, content, thumbnail, cat_id, tags, author_name) {
      $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.post(`/elgomhoria/admin/articles/edit`, {
                title: title,
                content: content,
                thumbnail: thumbnail,
    ...
    async getTagSearch(search_words) {
        try {
            const response = await axios.post(`/elgomhoria/admin/tags/search`, {
    ...
    async getSearchImages(search_words) {
        try {
            const response = await axios.post(`/elgomhoria/admin/images/search?page=${this.page}`, {
                search_words: search_words,
            },
            );
            if (response.data.status === true) {
    ...
    async getArticle() {
        $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.post(`/elgomhoria/admin/article`, {
                article_id: this.article_id
            },
            );
            if (response.data.status === true) {
    ...
    async getImages() {
        $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.get(`/elgomhoria/admin/images/get_images?page=${this.page}`
            );
            if (response.data.status === true) {
                $('.loader').fadeOut()
                this.images = response.data.data.data
    ...
    async uploadImage(image) {
        $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.post(`/elgomhoria/admin/articles/upload`, {
                img: image,
            },
            {
                headers: {
    ...
    chooseImage(imagePath) {
        this.choosed_img = '/elgomhoria/dashboard/images/uploads/' + imagePath;
    },
    previewThumbnail () {
  ...
  mounted() {
    ...
    $("#thumbnail").change(function () {
        ...
        if ($.inArray(fileType, validImageTypes) < 0) {
            ...
            $("#preview").attr(
                "src",
                this.thumbnail_path ? '/elgomhoria/dashboard/images/uploads/Articles_thumbnail/' + this.thumbnail_path : "/dashboard/images/add_image.svg"
            );
