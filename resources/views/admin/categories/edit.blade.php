...
<div class="card" id="edit_cat">
    <div class="card-body">
        ...
        <div>
            ...
            <div class="pop-up show-images-pop-up card" v-if="showImages" style="min-width: 90vw;height: 90vh;padding: 20px;display: flex;flex-direction: column;justify-content: space-between;gap: 1rem;">
                ...
                <div class="imgs p-2 gap-3" v-if="images && images.length" style="display: flex;grid-template-columns: repeat(auto-fit, minmax(250px, 1fr));flex-wrap: wrap;height: 100%;overflow: auto;">
                    <div class="img" @dblclick="previewThumbnail" @click="this.choosed_img = '/elgomhoria/dashboard/images/uploads/' + img.path" v-for="(img, index) in images" :key="img.id" style="width: 260px;height: 230px;overflow: hidden;padding: 10px;border-radius: 1rem;box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;">
                        <img :src="'/elgomhoria/dashboard/images/uploads/' + img.path" id="preview" alt="img logo" style="width: 100%;height: 100%;object-fit: contain;">
                    </div>
                </div>
            ...
            <div class="w-100 mb-3 d-flex gap-2">
                ...
                <div class="w-25">
                    <div class="w-100 h-100 p-3 d-flex justify-content-center align-items-center form-control" style="max-height: 170px;"  @click="this.showImages = true">
                        <img :src="preview_img ?  preview_img : (thumbnail_path ? thumbnail_path : '/elgomhoria/dashboard/images/add_image.svg')" id="preview" alt="img logo" style="width: 100%; max-width: 100%;object-fit: contain;height: 100%;">                                                
                    </div>
                </div>
            </div>
        ...
        @else
        @php
            return redirect('/elgomhoria/admin/categories/preview');
        @endphp
        @endif
    </div>
</div>
...
createApp({
  ...
  methods: {
    async save(category_translations, main_name, description, thumbnail) {
      ...
      try {
        const response = await axios.post(`/elgomhoria/admin/categories/edit`, {
        ...
        if (response.data.status === true) {
          ...
          setTimeout(() => {
            $('#errors').fadeOut('slow')
            window.location.href = '/elgomhoria/admin/categories'
          }, 2000);
        } else {
          $('.loader').fadeOut()
          document.getElementById('errors').innerHTML = ''
    ...
    async getLanguages() {
        $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.post(`/elgomhoria/admin/categories/get-languages`, {
            },
            );
            if (response.data.status === true) {
                $('.loader').fadeOut()
    ...
    async getCategories() {
        $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.post(`/elgomhoria/admin/categories/`, {
            },
            );
            if (response.data.status === true) {
                $('.loader').fadeOut()
    ...
    async getCategory() {
        $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.post(`/elgomhoria/admin/category`, {
    ...
    async getNameTranslations() {
        $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.post(`/elgomhoria/admin/category/names`, {
                category_id: this.category_id
            },
            );
            if (response.data.status === true) {
  ...
  mounted() {
    ...
    this.getNameTranslations()
        ...
        if ($.inArray(fileType, validImageTypes) < 0) {
            ...
            $("#preview").attr(
                "src",
                this.thumbnail_path ? '/elgomhoria/dashboard/images/uploads/categories_thumbnail/' + this.thumbnail_path : "/dashboard/images/add_image.svg"
            );
            $(".photo_group i").removeClass("fa-edit").addClass("fa-plus");
        } else {
            // display image preview
