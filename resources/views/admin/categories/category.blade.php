...
<div class="d-flex gap-2 justify-content-between mb-3">
    <div class="w-75">
        ...
        <a href="/elgomhoria/admin/categories/edit/{{ $category->id }}" class="btn btn-success">Edit Category</a>
    </div>
    <div class="img card p-2" style="max-height: 150px; max-width: 140px">
        <img src="{{$category->thumbnail_path ?  $category->thumbnail_path : '/elgomhoria/dashboard/images/add_image.svg' }}" id="preview" alt="img logo" style="width: 100%; max-width: 100%;object-fit: contain;height: 100%;">                                                
    </div>
</div>
@endif
                ...
                <tr>
                    ...
                    <td class="border-bottom-0">
                        <div class="d-flex gap-2">
                            <a href="/elgomhoria/admin/categories/edit/{{ $cat->id }}" class="btn btn-secondary p-2"><h4 class="ti ti-edit text-light m-0 fw-semibold"></h4></a>
...
createApp({
    ...
    methods: {
        async update(lang_id, lang_symbol, lang_name) {
            ...
            try {
                const response = await axios.post(`/elgomhoria/admin/languages/edit`, {
                    lang_id: lang_id,
        ...
        async deletCat(cat_id) {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/elgomhoria/admin/categories/delete`, {
                    cat_id: cat_id,
                },
                );
                if (response.data.status === true) {
        ...
        async getSearch(search_words) {
            try {
                const response = await axios.post(`/elgomhoria/admin/categories/search`, {
